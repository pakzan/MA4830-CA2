...
void f_Malloc(void){
	int i;
  if((ch = (wave_para*)malloc(2 * sizeof(wave_para))) == NULL) {
    ...
    if (clock_gettime(CLOCK_REALTIME,&start1)==-1){
      ...
      exit(EXIT_FAILURE);
        ...
        for (i=0; i<NO_POINT; i++){
          current1[i]= (sine_wave[i] * ch[0].amp)*0.6 + (ch[0].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
       }
       for (i=0; i<NO_POINT; i++){
        ...
        case 2 :
        for (i=0; i<NO_POINT; i++){
          current1[i]= (sq_wave[i] * ch[0].amp)*0.6 + (ch[0].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
        }
        for (i=0; i<NO_POINT; i++){
          out16(DA_CTLREG,0x0a23);			// DA Enable, #0, #1, SW 5V unipolar
          out16(DA_FIFOCLR, 0);				// Clear DA FIFO  buffer
        ...
        case 3 :
        for (i=0; i<NO_POINT; i++){
          current1[i]= (saw_wave[i] * ch[0].amp)*0.6 + (ch[0].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
        }
        for (i=0; i<NO_POINT; i++){
          out16(DA_CTLREG,0x0a23);			// DA Enable, #0, #1, SW 5V unipolar
          out16(DA_FIFOCLR, 0);				// Clear DA FIFO  buffer
        ...
        case 4 :
        for (i=0; i<NO_POINT; i++){
          current1[i]= (tri_wave[i] * ch[0].amp)*0.6 + (ch[0].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
       }
       for (i=0; i<NO_POINT; i++){
          out16(DA_CTLREG,0x0a23);			// DA Enable, #0, #1, SW 5V unipolar
          out16(DA_FIFOCLR, 0);				// Clear DA FIFO  buffer
   ...
   //printf("wave2\n");
    ...
    if (clock_gettime(CLOCK_REALTIME,&start2)==-1){
     ...
     exit(EXIT_FAILURE);
        ...
        case 1 :
        for (i=0; i<NO_POINT; i++){
          current2[i]= (sine_wave[i] * ch[1].amp)*0.6 + (ch[1].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
          }
        for (i=0; i<NO_POINT; i++){
        ...
        case 2 :
        for (i=0; i<NO_POINT; i++){
          current2[i]= (sq_wave[i] * ch[1].amp)*0.6 + (ch[1].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
        }
         for (i=0; i<NO_POINT; i++){
          out16(DA_CTLREG,0x0a43);			// DA Enable, #0, #1, SW 5V unipolar
          out16(DA_FIFOCLR, 0);				// Clear DA FIFO  buffer
        ...
        case 3 :
        for (i=0; i<NO_POINT; i++){
          current2[i]= (saw_wave[i] * ch[1].amp)*0.6 + (ch[1].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
        }
         for (i=0; i<NO_POINT; i++){
          out16(DA_CTLREG,0x0a43);			// DA Enable, #0, #1, SW 5V unipolar
          out16(DA_FIFOCLR, 0);				// Clear DA FIFO  buffer
        ...
        case 4 :
        for (i=0; i<NO_POINT; i++){
          current2[i]= (tri_wave[i] * ch[1].amp)*0.6 + (ch[1].mean*0.8 + 1.2)*0x7fff/5 * 2.5 ;			// add offset +  scale
       }
         for (i=0; i<NO_POINT; i++){
          out16(DA_CTLREG,0x0a43);			// DA Enable, #0, #1, SW 5V unipolar
          out16(DA_FIFOCLR, 0);				// Clear DA FIFO  buffer
...
while(true){
	...
	//clear console screen
        system("clear");
	if(pthread_create(&thread[0], NULL, &t_ScreenOutput, NULL)){
...
void signal_handler(){
  ...
  for(t=0;t<NUM_THREADS-1;t++){
    ...
    else printf("Thread 2 is killed earlier.\n")	;
	delay(100);
  }
  if (t ==  NUM_THREADS-1) 
  {
