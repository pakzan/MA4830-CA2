...
float f_ValidValue(float value, float min, float max)
{
...
void f_ReadFile(char *filename, FILE *fp)
{
	...
	if (count == 8)
	{
		...
		for (i = 0; i < 2; i++)
		{
			wave[i] = f_ValidValue(temp_wave[i], 1, 4);
			ch[i].amp = f_ValidValue(temp_ch[i].amp, 0, 5);
			ch[i].mean = f_ValidValue(temp_ch[i].mean, 0, 2);
			ch[i].freq = f_ValidValue(temp_ch[i].freq, 0.5, 5);
		}
